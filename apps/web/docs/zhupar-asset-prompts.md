...
| `oyu-icons` | **принят**, разрезан на четыре файла по четвертям листа |
| `hero` | **принят.** Пара стоит, петля 0,84 по SSIM, тихое поле под текст y 80–240. VP9 CRF 33, 820 КБ |
| `story-rings` | **принят** без оговорок |
| `story-table` | **принят.** Пришёл вертикальным; поперёк пустой скатерти шёл шов на 45 уровней темнее ткани — строки 440–500 сведены вертикальным градиентом, перепад после 0,8 уровня на 8 строк |
| `ground-paper` | **принят**, сгенерирован безопасным промптом ниже. Прежний промпт фильтр генератора блокировал, вероятно, на словах «rag» и «stains» |
| `envelope` | **принят со второго промпта**, без печати. Движение 0,38–2,96 с, обрезан 0,25–3,0 |

Безопасный промпт фона, который прошёл фильтр:

```
A seamless flat background texture of plain ivory watercolour paper, scanned
straight on, filling the whole square. Fine subtle grain, soft even light,
very low contrast, no marks, no edges, no shadows, no gradient. Square 1:1.
