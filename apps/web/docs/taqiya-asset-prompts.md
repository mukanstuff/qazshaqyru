...
**Итог 2026-09-13: принята полоса в двух сросшихся рядах.** «Exactly one row» не
сработал и здесь. Разрезать нельзя: на самой узкой строке между рядами 24
штриха, заливка 52%. Ставится целиком, пропорция файла 1370×351.
...
| `hero-taqiya` | принят. Подпись не на размытую ткань внизу арки — там светлого тихого поля нет, пятый процентиль 0,28–0,51. Белым по тёмно-синим рукавам, полоса ровная, текстура 0,015–0,026, или над аркой |
| `story-shapan` | принят **только кропом** груди и пояса, `extract 200,120,496×660`. Целиком в овале это мальчик без головы в портретной рамке: овал обещает лицо, а в нём срез по шее и подбородок сверху. Кроп убирает и подбородок, и разный фон слева и справа |
| `ground-linen` | принят |
...
| `envelope` | **принят владельцем.** Поля сверху и снизу, 44% высоты под конвертом, ему не мешают, а под надписи даже удобны; дотягивать конверт до краёв не нужно. Известный изъян: печать трескается на клапане, а под ней видна вторая, целая. **Собрано:** ролик обрезан 0,30–1,10 с — трещина на 0,58, вторая печать появляется с ~1,15 — и шлюз отдаёт страницу за 0,5 с до конца, то есть сразу после трещины. Имя и дата стоят в светлом поле над конвертом (`layout: 'split'`): внизу стопке из имени, даты и кнопки не хватало места, и имя ложилось на край конверта |
...
### 10б. `envelope` — не понадобился, владелец принял первый конверт
...
## 4. `oyu-arrow` — разделитель, 5:1 — СНЯТ

**Не генерировать.** Четыре дубля из четырёх пришли европейской кованой
решёткой: стрелы, лилии, волосяные завитки. Причина в промпте ниже, а не в
удаче — он описывает мотив, которого в казахском орнаменте нет, и модель
собрала его из того, что знает. Разбор в `template-playbook.md`. Роль
разделителя в шаблоне берёт медальон в малом масштабе, как у конкурентов.
Промпт оставлен как пример того, как не надо.
