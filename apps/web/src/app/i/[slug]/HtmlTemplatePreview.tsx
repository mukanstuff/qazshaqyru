/**
 * Server component — renders an HTML-engine template in the preview context.
 * Phone frame with dark Figma-like background, catalog/back button top-left,
 * Edit button at bottom-center.
 */

import { Metadata } from 'next';
import Link from 'next/link';
import { getHtmlTemplateDescriptor } from '@/lib/templates/manifests/index';
import { resolveHtmlTemplateData } from './HtmlGuestPage';
import HtmlTemplateFrame from './HtmlTemplateFrame';
import { EditButton } from './EditButton';
import type { Locale } from '@/lib/templates/html-engine/types';

interface HtmlTemplatePreviewProps {
  templateSlug: string;
  templateTitle: string;
  backHref: string;
  /** Called when user clicks "Редактировать". Use in Client Components. */
  onEdit?: () => void;
  locale?: Locale;
}

export async function generateMetadata(props: HtmlTemplatePreviewProps): Promise<Metadata> {
  const { templateSlug, locale = 'ru' } = props;
  const resolved = await resolveHtmlTemplateData('demo', templateSlug, true, locale);
  if (!resolved) return { title: props.templateTitle };
  return { title: resolved.meta.title };
}

export default async function HtmlTemplatePreview(props: HtmlTemplatePreviewProps) {
  const { templateSlug, backHref, onEdit, locale = 'ru' } = props;

  const resolved = await resolveHtmlTemplateData('demo', templateSlug, true, locale);

  return (
    <div className="relative h-screen overflow-hidden" style={{ background: '#1c1c1e' }}>
      {/* Top-left: back to catalog */}
      <Link
        href={backHref}
        className="absolute top-3 left-3 z-10 flex items-center gap-1.5 rounded-full bg-emerald-600 px-5 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M19 12H5M12 5l-7 7 7 7"/>
        </svg>
        В каталог
      </Link>

      {/* Phone frame — centered */}
      <div className="flex h-full items-center justify-center overflow-hidden">
        {resolved ? (
          <HtmlTemplateFrame html={resolved.html} />
        ) : (
          <p className="text-white/40">Шаблон &laquo;{templateSlug}&raquo; не загружен</p>
        )}
      </div>

      {/* Bottom-center: Edit button */}
      {onEdit ? (
        <EditButton onClick={onEdit} />
      ) : (
        <Link
          href="/create"
          className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-2 rounded-full bg-emerald-600 px-7 py-2.5 text-sm font-medium text-white transition-colors hover:bg-emerald-700"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
          </svg>
          Редактировать
        </Link>
      )}
    </div>
  );
}
