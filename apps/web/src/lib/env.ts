import { z } from 'zod';
import { getKzSmsMissingCredentials } from './sms';
import { describeUploadStorage } from './upload-storage';
import { validateS3UrlSeparation } from './s3';
import { getKaspiWebhookUrl, isKaspiWebhookReady } from './payment-provider-config';
...
export function auditProductionEnv(env: NodeJS.ProcessEnv = process.env): EnvCheckItem[] {
  ...
  if (!appUrl) {
    pushCheck(items, 'APP_URL', 'error', 'APP_URL is required (https://your.domain or http://VPS_IP before domain)');
  } else {
    try {
      ...
      if (url.protocol === 'http:') {
        ...
        pushCheck(items, 'APP_URL', 'ok', `APP_URL is ${appUrl}`);
      }
    } catch {
